const Command = require("../command.js");
// NOTE: If at any time, you want to focus on the output from a single test, feel free to comment out all the others.
//       However, do NOT edit the grading tests for any reason and make sure to un-comment out your code to get the autograder to pass.

//test 1
describe("Command class", function () {
  it("throws error if command type is NOT passed into constructor as the first parameter", function () {
    expect(function () {
      new Command();
    }).toThrow(new Error("Command type required."));
  });
});

//Test 2
describe("constructor sets command type", function () {
  it("corectly sets command type", function () {
    let modeCommand = new Command("MODE_CHANGE", "LOW_POWER");
    expect(modeCommand.commandType).toBe("MODE_CHANGE");
  });
});

//Test 3
describe("constructor sets a value passed in as the 2nd argument", function () {
  it("coorectly sets the value property", function () {
    let modeCommand = new Command("MOVE", 12000);
    expect(modeCommand.value).toBe(12000);
  });
});
